	...
	if(elem.length!=0){
		...
		if(navHover==true){	//if the option is true I show the next and prev button only on mouse over
			if(isMobile()){
				...
				elem.hover(function(){
					$('#pix_prev, #pix_next').stop(true,false).animate({opacity:0.5},800);
				},function(){
		...
		if (($.browser.msie && $.browser.version < 9) || opts.loader == 'bar') {	//IE8- has some problems with canvas, I prefer to use a simple loading bar in CSS
			...
			canvas.css({'position':'absolute', 'left':-25, 'z-index':1001, 'height':opts.barStroke, 'width':0, 'background-color':opts.loaderColor});
		} else {
